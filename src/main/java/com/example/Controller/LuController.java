...
public class LuController {
    ...
    public SseEmitter aiChat(String question) {
        ...
        luService.aiChat(question,sseEmitter);
        return sseEmitter;
    }
    //    查询单个菜品
    @GetMapping("/dish/{id}")
...
//            事务回滚，删除图片
    ...
    public Result updateDish(
        ...
        if(image != null){
            oldImageUrl = luService.searchDish(id).getImage();/* 获取旧图片地址 */
    ...
    public Result deleteDish(@PathVariable Integer id) throws Exception {
        log.info("删除菜品");
//        首先获取图片地址
        String imageUrl = luService.searchDish(id).getImage();
        try {
            luService.deleteDish(id);
        } catch (Exception e) {
...
//            拓展接口，出现问题首先储存图片地址
    ...
    public Result searchDishes(
        ...
        List<Dish> dishes = luService.searchDishes(name,price);
        log.info("条件搜索");
        return Result.success(dishes);
    }
}
